...
void main() {
    ...
    if (length(light_weight) < 1.00) { 
        ...
        // gl_FragColor = color * (sin(gl_FragCoord.x) + sin(gl_FragColor.y));
    }
    ...
    // if (length(light_weight) < 1.00) { 
    //     if (mod(gl_FragCoord.x + gl_FragCoord.y, 15.0) == 0.0) { 
    ...
    // } 
    // if (length(light_weight) < 0.50) { 
    //     if (mod(gl_FragCoord.x + gl_FragCoord.y - 5.0, 15.0) == 0.0) { 
    //         gl_FragColor = vec4(0.0, 0.0, 0.0, 1.0); 
    //     } 
    // } 
