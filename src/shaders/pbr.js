...
import {cubemapLoaded} from '../mario'
import {cubemapDiffLoaded} from '../mario'
import {brdfLUTLoaded} from '../mario'
...
export default function(renderer, scene, camera) {
    ...
    const Shader = {
        initGUI: function(gui) {
            ...
            gui.add(options, 'roughness', 0.0, 1.0).onChange(function(val){
                Shader.material.uniforms.u_roughness.value = val;
            });

            gui.add(options, 'metalness', 0.0, 1.0).onChange(function(val){
                Shader.material.uniforms.u_metalness.value = val;
            });
        ...
        material: new THREE.ShaderMaterial({
            ...
            fragmentShader: require('../glsl/pbr_frag.glsl')
        })
    }
    ...
    brdfLUTLoaded.then(function(texture) {
        Shader.material.uniforms.brdfLUT.value = texture;
    });

    cubemapLoaded.then(function(texture) {
        scene.background = texture;
        Shader.material.uniforms.cubeTexture.value = texture;
    });
